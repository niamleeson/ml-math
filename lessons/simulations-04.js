    ...
    stages: [
        ...
        steps: [
          ...
          { type: "decide", prompt: "23% of objects are smaller than 32px. What does that imply?",
            options: [
              ...
              { label: "Keep input resolution high and add multi-scale features so small items survive downsampling", best: true, feedback: "right — the fix has to restore the detail downsampling destroys. Higher input resolution gives small objects more pixels to begin with, and a feature pyramid (FPN) detects on several scales so small items are found on the high-resolution shallow maps before they vanish. This directly targets the 23% small-box slice instead of hoping the average hides it." }
            ] }
        ]
      },
      {
      ...
      },
      {
        ...
        data: {
          ...
          note: `Alert bands come from each metric's normal variance: a move beyond ~3σ of the recent baseline is unlikely by chance (CLT), so it signals a real shift, not noise.`
        },
        symbols: [
    ...
    stages: [
      {
        ...
        symbols: [
          ...
          { sym: "prevalence", desc: "fraction of the population that is positive (~$4\\%$ here); why accuracy and PPV are so easily fooled." }
        ],
        steps: [{
    ...
    stages: [
      ...
      },
      {
        ...
        data: {
          ...
          note: `The state estimator (a recursive Bayes/HMM-style filter) predicts each object's next state and corrects it with new detections, so tracks survive a dropped frame. Fusion must respect each sensor's geometry — you align in 3D space, not by averaging pixels.`
        },
        symbols: [
      ...
      },
      {
        ...
        insight: `<b>Constrain the domain, keep two backstops, expand on evidence.</b> The launch is a <b>6-mile loop at ≤25 mph</b> with a safety operator and the fail-safe armed — week 1 logs <b>0 collisions, 5 precautionary takeovers, 2 minimal-risk stops</b>, all signals the layered safety net is working. The geofence bounds <i>which</i> situations can occur (a small, mapped ODD), the operator and fail-safe bound <i>what happens</i> when something unexpected does. A city-wide driverless day-one launch would discard every one of those bounds at once.`,
        data: {
