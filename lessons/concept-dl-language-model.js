  ...
  window.LESSONS.push({
    ...
    bigIdea:
      `<p>A <b>language model</b> assigns a probability to a whole sentence. "the cat sat on the mat" should score higher than "mat the on cat sat the" — the model has learned what real English looks like.</p>
       <p><b>Analogy — the phone keyboard.</b> When your phone suggests the next word as you type, it is acting like a language model: given the words so far, it ranks what is likely to come next. A good keyboard rarely surprises you; a bad one keeps suggesting nonsense. Perplexity is just a number that measures that surprise.</p>
       <p>Scoring a whole sentence at once is hopeless: there are astronomically many possible sentences, so we could never have seen each one to count it. So we <b>factor</b> the sentence probability into a product of next-word probabilities, one word at a time, using the <b>chain rule</b> of probability (a rule that says a joint probability can be rebuilt by multiplying one-step-at-a-time conditional probabilities). Each factor is "how likely is this word, given everything before it."</p>
       <p>Those "everything before it" histories are still too long to count. The <b>n-gram approximation</b> chops the history down to the last $n-1$ words (an <b>n-gram</b> is just a run of $n$ words in a row) and estimates each probability by a simple <b>count ratio</b> — how often we saw the run, divided by how often we saw its start. Then <b>perplexity</b> tells us how good the model is: how surprised it is, on average, by held-out text (text set aside and never trained on). Lower is better.</p>`,
