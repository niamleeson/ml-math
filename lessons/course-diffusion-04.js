  ...
  window.LESSONS.push({
    ...
    sections: [
      ...
      },
      {
        ...
        <p>This is a Markov chain: the next state's distribution depends only on the current value $x_{t-1}$. Its transition kernel, written as a distribution — recall $\\mathcal{N}(x;\\, \\mu, \\sigma^2)$ from <a onclick="App.open('prob-normal')">part 3's Gaussian</a>, the bell curve with center $\\mu$ and spread $\\sigma^2$:</p>
    ...
    practice: [
      {
        ...
        steps: [
          ...
          { do: "Peel off $b$ the same way: $p(a, b) = p(a)\\, p(b \\mid a)$, so $p(a, b, c) = p(a)\\, p(b \\mid a)\\, p(c \\mid a, b)$.", why: "The identical definition applied one variable earlier, then substituted in — equals may replace equals." },
          { do: "Backward, peel off $a$ first, then $b$: $p(a, b, c) = p(b, c)\\, p(a \\mid b, c) = p(c)\\, p(b \\mid c)\\, p(a \\mid b, c)$.", why: "The chain rule is bookkeeping, not an assumption — the peeling order is free, so the same joint can be told from $c$ backwards." },
      ...
      },
      {
        ...
        steps: [
          ...
          { do: "Evaluate: $0.7^{10} \\approx 0.0282$, so $m_{20} \\approx 0.28$.", why: "Arithmetic — the starting value 10 has been squashed to about a fortieth of itself." },
