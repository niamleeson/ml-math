  ...
  window.LESSONS.push({
    ...
    sections: [
      ...
      },
      {
        ...
        <p>Finally, the famous <b>68-95-99.7 rule</b>, with a numeric read. For any Gaussian: about $68.3\\%$ of draws land within one $\\sigma$ of the center, about $95.4\\%$ within two, and about $99.7\\%$ within three. For $\\mathcal{N}(5, 4)$ — center 5, variance 4, so $\\sigma = 2$ — that means: about two thirds of draws land in $[3, 7]$, about 19 out of 20 land in $[1, 9]$, and nearly all land in $[-1, 11]$. Draws four or more sigmas out are rare enough that you may go a hundred thousand draws without seeing one. You met this curve briefly in <a onclick="App.open('prob-normal')">the normal distribution lesson</a>; here we go further and learn its algebra.</p>
      ...
      },
      {
        ...
        <ol>
          ...
          <li>Read the right side as a reparameterization with center $a\\mu$ and stretch $a\\sigma$: $\\; aX \\sim \\mathcal{N}(a\\mu, (a\\sigma)^2)$. <b>Why:</b> "a constant plus a constant times standard noise" is exactly the pattern that defines a Gaussian with that mean and that spread.</li>
          <li>Expand the square: $(a\\sigma)^2 = a^2\\sigma^2$. <b>Why:</b> the square of a product is the product of squares.</li>
        </ol>
        <p>Real numbers: if $X \\sim \\mathcal{N}(1, 4)$, then $3X \\sim \\mathcal{N}(3, 36)$. The standard deviation went from 2 to 6 (times $a = 3$), so the variance went from 4 to 36 (times $a^2 = 9$). This square trips people up constantly, so say it once more: <b>spread scales like $a$, variance scales like $a^2$.</b> It also explains, in advance, an oddity you will meet in Part 6: to make a variable's variance come out as $1 - \\beta_t$, the recipe multiplies by $\\sqrt{1 - \\beta_t}$ — the square root is there precisely because variances pick up the square of the scale.</p>
      ...
      },
      {
        ...
        <ol>
          ...
          <li>Fold the $x$-free pieces $\\frac{\\mu_1^2}{2\\sigma_1^2} + \\frac{\\mu_2^2}{2\\sigma_2^2}$ into the constant. <b>Why:</b> no $x$ means no effect on the shape, only on the overall scale.</li>
