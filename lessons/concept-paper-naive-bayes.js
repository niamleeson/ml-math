...
(function () {
  window.LESSONS.push({
    ...
    symbols: [
      ...
      { sym: "$P(c_j\\mid d_i)$", desc: "the <b>posterior</b>: how probable class $c_j$ is given the document. The classifier returns the class with the largest posterior." },
    ...
    formula: `$$ \\text{(multinomial likelihood, Eqn. 5)}\\quad P(d_i\\mid c_j;\\theta)=P(|d_i|)\\,|d_i|!\\prod_{t=1}^{|V|}\\frac{P(w_t\\mid c_j;\\theta)^{N_{it}}}{N_{it}!} $$
...
$$ \\text{(classify, Eqn. 7)}\\quad P(c_j\\mid d_i;\\hat\\theta)=\\frac{P(c_j\\mid\\hat\\theta)\\,P(d_i\\mid c_j;\\hat\\theta_j)}{P(d_i\\mid\\hat\\theta)} $$`,
    whatItDoes:
