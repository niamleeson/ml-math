...
(function () {
  window.LESSONS.push({
    ...
    evaluation:
      `<p><b>The metric &amp; benchmark.</b> This is a theory paper, so "working" has two faces. (1) <b>Correctness of the rule:</b> your scratch 1-NN predictions must match the <code>torch.cdist</code> + <code>argmin</code> oracle exactly &mdash; the metric is the fraction of queries where the predicted label agrees (target: $100\\%$), plus <code>torch.allclose</code> on the squared-distance matrix. (2) <b>The bound holds:</b> on a problem where you can compute the Bayes error $R^*$ in closed form (two 1-D Gaussians, equal priors), measure the large-sample 1-NN error $R$ and check the Cover-Hart sandwich $R^* \\le R \\le 2R^*$. The "no-skill" baseline is majority-class guessing, which errs at $1 - \\max_i \\eta_i$ (for balanced two-class data, $50\\%$); a correct 1-NN must beat it whenever $R^* \\lt 0.5$.</p>
       <ul>
        <li><b>Sanity checks before the full run.</b> Run the worked example by hand first: query $(2,0)$ over $\\{(0,0),(3,4),(1,0)\\}$ must give squared distances $[4, 17, 1]$, nearest index $2$, predicted label $0$. Check the distance matrix shape is (queries $\\times$ train). Feed a query that <i>equals</i> a training point: its squared distance to that point must be $\\approx 0$ (clamp tiny negatives from the $\\lVert a-b\\rVert^2$ expansion) and it must predict that point's own label. Both assertions (<code>allclose</code> on distances, exact label match) must fire on random toy data before trusting anything.</li>
        <li><b>Expected range.</b> The rule check is binary &mdash; <i>exact</i> agreement with the oracle, or you have a bug (wrong broadcast axis, <code>argmin</code> over the wrong dimension). For the bound, the paper's own worked triangle example (&sect;VII) gives an exact target: $R^* = \\tfrac14$, $R = \\tfrac13$, with $R(n) = \\tfrac13 + \\frac{1}{(n+1)(n+2)} \\to \\tfrac13$ &mdash; a correct finite-$n$ simulation should converge to $\\tfrac13$ as $\\sim 1/n^2$. In the Gaussian sweep, every measured $(R^*, R)$ pair should sit inside $[R^*,\\, 2R^*]$ (rule of thumb: allow $\\pm 0.01$ Monte-Carlo slack from finite test sets; a point clearly <i>below</i> $R^*$ means a leak, clearly above $2R^*$ means too few training points).</li>
        <li><b>Ablation &mdash; prove the key idea earns its keep.</b> The central claim is that <i>closeness</i> carries the label. Break it: (a) replace <code>argmin</code> with <code>argmax</code> (pick the <i>farthest</i> point) and the error should rocket toward (and past) the majority-class baseline &mdash; if it does not, your distances are wrong. (b) Shrink $n$ toward $1$: the asymptotic guarantee dissolves and $R$ rises above $2R^*$, confirming the bound is genuinely large-sample (Eqn. 11). (c) Standardize vs. blow up one feature's scale by $1000\\times$: the "nearest" neighbor becomes decided by that one axis and accuracy drops &mdash; confirming the metric $d$ is load-bearing.</li>
        <li><b>Failure signals &amp; what they mean.</b> Error stuck at the majority-class rate &rarr; labels shuffled relative to points, or you indexed <code>ytr</code> with the wrong axis. Measured $R$ <i>below</i> $R^*$ &rarr; train/test leakage (the same points are in both sets, so each query finds itself at distance $0$). <code>allclose</code> fails by a tiny amount only &rarr; you forgot the $\\sqrt{\\cdot}$ relationship ($d2$ vs $d$: oracle is <code>cdist</code><b>$^2$</b>), or float error on identical points &mdash; clamp to $0$. $R$ comfortably above $2R^*$ &rarr; too few training points, the neighbor is not yet close (re-run with larger $n$). The CODEVIZ band plot is the diagnostic: a green point poking above the grey $2R^*$ line is the canonical "not enough data" signal.</li>
       </ul>`,
