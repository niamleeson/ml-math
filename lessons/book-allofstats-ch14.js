  ...
  B({
    ...
    sections: [
      { h: "What a random vector is", body:
        "<p>A multivariate model studies several random quantities at once, packed into a single column called a <strong>random vector</strong> $X = (X_1,\\dots,X_k)^T$ — a list of $k$ random numbers stacked vertically. The chapter first reviews a little linear-algebra notation:</p>" +
        "<table class=\"extable\"><thead><tr><th>notation</th><th>meaning</th></tr></thead><tbody><tr><td class=\"row-h\">$x^T y$</td><td>inner product $\\sum_j x_j y_j$</td></tr><tr><td class=\"row-h\">$|A|$</td><td>determinant</td></tr><tr><td class=\"row-h\">$A^T$</td><td>transpose of $A$</td></tr><tr><td class=\"row-h\">$A^{-1}$</td><td>inverse of $A$</td></tr><tr><td class=\"row-h\">$I$</td><td>identity matrix</td></tr><tr><td class=\"row-h\">$\\text{tr}(A)$</td><td>trace: sum of diagonal entries</td></tr><tr><td class=\"row-h\">$A^{1/2}$</td><td>square-root matrix</td></tr></tbody></table>" +
      ...
      ,
      { h: "Worked correlation example from the computer experiment", body:
        "<p>The chapter's exercise uses a bivariate Normal with mean and covariance</p><ul class=\"steps\"><li>$\\mu=(3,8)^T$ and $\\Sigma=\\begin{pmatrix}1 & 1 \\\\ 1 & 2\\end{pmatrix}$.</li><li>The marginal standard deviations are $\\sigma_1=\\sqrt{1}=1$ and $\\sigma_2=\\sqrt{2}=1.4142$.</li><li>The population correlation is $\\rho=\\Sigma_{12}/(\\sigma_1\\sigma_2)=1/(1\\cdot\\sqrt{2})=0.7071$.</li><li>For the requested $n=100$ simulation, Fisher's standard error is $1/\\sqrt{100-3}=0.1015$.</li></ul><pre><code class=\"language-python\"># Chapter 14, Exercise 5: simulate N(mu, Sigma), estimate rho, Fisher CI\nimport numpy as np\n\nrng = np.random.default_rng(13)\nmu = np.array([3.0, 8.0])\nSigma = np.array([[1.0, 1.0], [1.0, 2.0]])\nrho_true = Sigma[0, 1] / np.sqrt(Sigma[0, 0] * Sigma[1, 1])\nX = rng.multivariate_normal(mu, Sigma, size=100)\nxbar = X.mean(axis=0)\nS = np.cov(X, rowvar=False, ddof=1)\nrho_hat = np.corrcoef(X, rowvar=False)[0, 1]\nz = 0.5 * (np.log(1 + rho_hat) - np.log(1 - rho_hat))\nlo_z, hi_z = z - 1.96/np.sqrt(97), z + 1.96/np.sqrt(97)\nci = (np.tanh(lo_z), np.tanh(hi_z))\nprint(rho_true)                         # 0.7071067812 (book parameters)\nprint(xbar, S, rho_hat, ci)             # simulated estimates</code></pre>" }
    ],
    takeaways: [
  ...
  B({
    ...
    sections: [
      ...
      { h: "Likelihood and the MLE", body:
        ...
        "<p>The appendix proof rewrites $\\sum_{i=1}^n (X^i-\\mu)^T \\Sigma^{-1}(X^i-\\mu)$ by adding and subtracting $\\overline{X}$ — the cross term vanishes because $\\sum_{i=1}^n (X^i-\\overline{X}) = 0$ — leaving a term in $(\\overline{X}-\\mu)$ plus a term that, using $\\text{tr}(AB)=\\text{tr}(BA)$ and the scalar-trace trick, collapses to $n\\,\\text{tr}(\\Sigma^{-1}S)$.</p><pre><code class=\"language-python\"># MLEs for the multivariate Normal (Theorem 14.3)\nimport numpy as np\n\n# Book's computer-experiment parameters: mu=(3,8), Sigma=[[1,1],[1,2]]\nrng = np.random.default_rng(14)\nX = rng.multivariate_normal([3, 8], [[1, 1], [1, 2]], size=100)\nn = len(X)\nxbar = X.mean(axis=0)                   # mu-hat = Xbar\nS_unbiased = np.cov(X, rowvar=False, ddof=1)\nSigma_hat = ((n - 1) / n) * S_unbiased  # MLE, Eq. 14.8\nprint(xbar)\nprint(Sigma_hat)</code></pre>" }
    ],
    takeaways: [
