  ...
  "search-ranking": {
    ...
    stages: [
      ...
      },
      {
        ...
        insight: `<b>Cheap signal, expensive anchor.</b> You assemble <b>58M query-doc pairs</b> from clicks but only <b>42K editorial grades</b> — clicks outnumber expert labels ~1,400 to 1. Clicks scale to the long tail; the 42K graded pairs (on a 0–4 scale) calibrate them so a click means "relevant", not just "shown at rank 1".`,
        data: {
        ...
        symbols: [
          ...
          { sym: "examination prob.", desc: "the modeled chance a user actually looked at a result at a given rank; clicks are reweighted by its inverse to debias them." },
          { sym: "grade 0–4", desc: "the editorial relevance scale, from 0 (irrelevant) to 4 (perfect) — the trusted anchor for the noisy click labels." }
        ],
        steps: [
      ...
      },
      {
        ...
        steps: [{
          ...
          options: [
            ...
            { label: "A large transformer scoring every candidate at full depth", feedback: "powerful for relevance but it breaks the latency budget at this stage. Running a deep transformer over thousands of candidates per query is far too slow and expensive to serve under a search SLA. Its place is RE-RANKING a small shortlist of a few dozen after a cheaper model has narrowed the field — not as the first-pass ranker over the whole candidate set." }
          ]
        }]
      },
      {
      ...
      },
      {
        ...
        symbols: [
          { sym: "NDCG@10", desc: "the position-discounted relevance of the top 10 results, normalized to [0,1] — the primary ranking metric." },
  ...
  "marketing-churn": {
    ...
    stages: [
      ...
      },
      {
        ...
        data: {
          caption: "Risk vs uplift — same high-risk decile, very different ROI",
  ...
  "ab-testing": {
    ...
    stages: [
      ...
      },
      {
        ...
        narrative: `<p>Before launching, decide how much traffic you need. Sample size depends on three things: the baseline conversion rate, the smallest effect worth detecting (the MDE), and the variance of the metric — roughly $n\\propto \\frac{\\sigma^2}{\\delta^2}$, so halving the effect you want to catch quadruples the users you need. Computing this up front fixes the finish line so you read the result exactly once, instead of peeking until randomness crosses the line.</p>`,
        ...
        symbols: [
          { sym: "$n$", desc: "required sample size per arm — how many users each variant needs to hit the target power." },
          { sym: "$\\sigma^2$", desc: "the variance of the metric; for a proportion $p$ it is $p(1-p)$, so the baseline rate sets it." },
          { sym: "$\\delta$ (MDE)", desc: "minimum detectable effect — the smallest lift you commit to being able to catch (here +0.6pp)." },
      ...
      },
      {
        ...
        symbols: [
          ...
          { sym: "$\\chi^2$", desc: "chi-square statistic comparing observed vs expected counts; large values mean an unlikely-by-chance imbalance." },
      ...
      },
      {
        ...
        symbols: [
          { sym: "CUPED", desc: "a variance-reduction method: adjust the outcome by a pre-experiment covariate to remove pre-existing noise." },
          { sym: "$\\theta$", desc: "the CUPED coefficient — how much of the covariate to subtract; chosen as $\\mathrm{Cov}(Y,X)/\\mathrm{Var}(X)$ to maximally cut variance." },
          { sym: "$\\rho$", desc: "correlation between the pre-period covariate and the outcome; variance drops by a factor of $\\rho^2$." },
          { sym: "covariate", desc: "a pre-randomization variable (prior conversion) tied to the outcome but unaffected by the treatment." }
        ],
        steps: [{
      ...
      },
      {
        ...
        insight: `<b>The metric dictates the test.</b> Conversion is a 0/1 Bernoulli variable with variance $p(1-p)$, so the principled choice is a <b>two-proportion z-test</b>. At <b>~58K users per arm</b> the CLT makes the difference in rates effectively normal, and a <b>95% CI on the lift</b> answers 'how big, and how sure?' — far more useful than a yes/no verdict at $\\alpha=0.05$.`,
        symbols: [
