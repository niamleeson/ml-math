...
<p><b>1. Estimate $p_{\\text{examine}}(\\text{rank})$ with a result-randomization experiment.</b> On ~1% of traffic, randomly shuffle the top results before showing them. Because a document now lands at each rank by chance (not because it was good), the click-through rate at each position reflects only how often that position is examined — not relevance. The CTR you measure at rank $r$ under randomization IS the estimate $p_{\\text{examine}}(r)$. (Where you can't randomize, an EM-based position-bias model recovers the same curve from organic logs by jointly fitting examination and relevance.)</p>
