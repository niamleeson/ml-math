  ...
  "fraud-anomaly": {
    ...
    stages: [
      ...
      },
      {
        ...
        insight: `<b>Match the model to the data's shape.</b> Tabular fraud signal is conjunctive — it's the <i>combination</i> of "$z>5$ AND new device AND 6 txns/hr" that's damning, not any single feature. Trees split on exactly these conjunctions for free, which is why gradient boosting tops nearly every tabular benchmark and why it'll hit ~$0.97$ AUC (Area Under the Curve) here while a single linear boundary can't. It also serves in well under the sub-$100$ms budget and hands you feature importances for free.`,
        symbols: [
          { sym: "AUC", desc: "area under the ROC (Receiver Operating Characteristic) curve — probability the model ranks a random fraud above a random legit txn (0.5 = chance, 1.0 = perfect)." },
    ...
    stages: [
      {
        ...
        symbols: [
          ...
          { sym: "$P(\\text{default})$", desc: "the probability the model assigns to this borrower defaulting; also written PD (Probability of Default)." }
        ],
        steps: [{
      ...
      },
      {
        ...
        symbols: [
          ...
          { sym: "AUC", desc: "area under the ROC (Receiver Operating Characteristic) curve — the chance the model ranks a random defaulter above a random repayer; $0.5$ is coin-flip, $1.0$ is perfect." }
        ],
        steps: [{
    ...
    stages: [
      ...
      },
      {
        ...
        data: {
          ...
          note: `Splitting <i>by campaign/sender</i> (not by row) keeps every variant of a blast on one side, so test spam is genuinely unseen. A leaky column makes offline AUC (Area Under the Curve) look magical and then vanishes live.`
        },
        symbols: [
      ...
      {
        phase: "Features", icon: "🧬", title: "Engineer features",
        narrative: `<p>Turn raw text into numbers a model can score. The classic spam representation is TF-IDF (Term Frequency–Inverse Document Frequency) over word and character n-grams: each message becomes a sparse vector of weighted term counts, which a Naive-Bayes or linear model reads as word-level evidence. But word counts miss paraphrased abuse, so you add embedding vectors for meaning and link/sender metadata to catch blast patterns the text alone hides.</p>`,
      ...
      },
      {
        ...
        symbols: [
          { sym: "AUC", desc: "area under the ROC (Receiver Operating Characteristic) curve: probability the model ranks a random spam above a random ham; $1.0$ is perfect, $0.5$ is chance." },
    ...
    stages: [
      {
        ...
        steps: [{
          ...
          options: [
            { label: "Predict the sign/size of next-period excess return, and judge by risk-adjusted return (Sharpe) net of costs", best: true, feedback: "right. The product is a P&L (Profit and Loss) stream, and what survives is consistency, not occasional big wins — so you judge by Sharpe $\\frac{E[r]}{\\sigma(r)}$, reward per unit of volatility, measured AFTER trading costs. Predicting the sign/size of the excess RETURN (not the price level) targets the quantity you can actually trade. Folding costs into the metric stops a strategy from 'winning' on paper by churning." },
            { label: "Predict the exact future price level and minimize MSE (Mean Squared Error)", feedback: "trap: price levels are near a random walk, so a tiny MSE on the level can still leave the period-to-period return essentially unpredictable. The model nails the level by echoing yesterday's price — an accurate-looking number with zero tradable edge once you difference it into returns." },
