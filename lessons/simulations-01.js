  ...
  "fraud-anomaly": {
    ...
    stages: [
      ...
      },
      {
        ...
        symbols: [
          { sym: "scale_pos_weight", desc: "the factor (332) that up-weights each fraud in the loss to offset the 332:1 class imbalance." },
    ...
    stages: [
      ...
      },
      {
        ...
        symbols: [
          ...
          { sym: "calibration", desc: "agreement between predicted PD and observed default frequency; here within $1.2\\%$ in every PD decile." }
        ],
        steps: [
      ...
      },
      {
        ...
        symbols: [
          { sym: "PSI", desc: "Population Stability Index — a single number measuring how much a feature's or score's distribution has drifted from training; $&gt;0.25$ flags a major shift." },
  ...
  "spam-moderation": {
    ...
    stages: [
      ...
      },
      {
        ...
        insight: `<b>TF-IDF turns a message into a sparse weighted bag of terms.</b> A term's weight is $\\text{tf}\\times\\text{idf}$: how often it appears in this message, scaled up if it's rare across the whole corpus and down if it's everywhere. So 'free', 'bit.ly', and 'reward' light up on spam while 'the' and 'and' get crushed to near-zero. Each message is a vector over a vocabulary of tens of thousands of terms but only a handful are non-zero — that sparsity is exactly what makes the Naive-Bayes baseline fast enough for millions of messages.`,
        data: {
        ...
        symbols: [
          ...
          { sym: "$\\times$", desc: "multiplication — the two factors are multiplied to form each term's weight." },
  ...
  "algorithmic-trading": {
    ...
    stages: [
      ...
      },
      {
        ...
        insight: `<b>Four signals, one bet.</b> The <b>4 highly-correlated signals ($|r|&gt;0.9$)</b> look like diversification but aren't — stack them and you've put roughly $4\\times$ the weight on the same underlying move, so a single regime shift hits all four at once. PCA finds the eigenvectors of the $\\Sigma$ covariance matrix: the first component (PC1) typically soaks up the shared variance (here the bulk of it), and the leftover PCs carry whatever independent signal remains. You trade the uncorrelated components, not the redundant raw signals.`,
        data: {
        ...
        symbols: [
          ...
          { sym: "$\\Sigma$", desc: "the covariance matrix of the signals — entry $(i,j)$ is how signal $i$ co-moves with signal $j$; PCA eigen-decomposes it." },
          { sym: "PC1 / PC2", desc: "the first/second principal components — the top eigenvectors of $\\Sigma$, the new uncorrelated factor directions; the loadings are their weights on each raw signal." }
        ],
        steps: [{
