  ...
  window.LESSONS.push({
    ...
    bigIdea:
      `<p>A sequence model (an RNN or Transformer decoder) picks output words one at a time. We don't want a good <i>first</i> word — we want the most probable <b>whole sentence</b>.</p>
       <p><b>Analogy — choosing a route.</b> Imagine driving across a city, deciding one turn at a time. <b>Greedy</b> is taking whatever turn looks best right now — but the road that looks fastest at the start can dump you onto a dead-end highway you can never leave. <b>Beam search</b> instead keeps a small handful of the most promising routes alive at once, follows each forward a step, and again keeps only the best handful. If your favourite first turn turns sour, you still have backups in play.</p>
       <p><b>Greedy decoding</b> takes the single highest-probability word at each step. That is <b>myopic</b> (short-sighted — it only looks one move ahead): a great first word can force a bad rest-of-sentence, and greedy can never take it back.</p>
       <p><b>Beam search</b> hedges. At every step it keeps the best <b>B</b> partial sentences (B is the <b>beam width</b> — literally how many candidate sentences-so-far we carry forward), expands each by trying every possible next word, then re-keeps only the best B of all those expansions. With $B = 1$ it <i>is</i> greedy (one route, no backups); larger $B$ explores more candidates but costs more compute.</p>`,

    buildup:
      `<p>Score a candidate sentence by the model's probability of that exact word sequence. We pick the sequence that maximizes it. Because multiplying many probabilities underflows (a chain of numbers below 1 multiplied together shrinks toward zero until the computer rounds it to exactly 0), we work in <b>log space</b>: the log of a product is the sum of logs, so we add log-probabilities instead of multiplying probabilities. ("log" here is the natural logarithm $\\ln$; the log of a number between 0 and 1 is negative, so each token contributes a negative number.)</p>
       <p>But summing logs has a side effect: every word adds another negative number, so a 10-word sentence almost always scores lower than a 3-word one. <b>Analogy:</b> it is like grading essays by total spelling mistakes instead of mistakes-per-page — a longer essay looks worse just for being longer, even if it is better written. That biases the search toward <b>short</b> outputs. The fix is to divide the summed log-probability by the length raised to a power, $T_y^{\\alpha}$ — <b>length normalization</b> — which converts the total into something close to a per-word average so length stops deciding the winner.</p>`,
