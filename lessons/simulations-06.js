    ...
    stages: [
      {
        ...
        data: {
          ...
          note: `Cross-entropy stays smooth and large when the model is wrong; MSE (Mean Squared Error)-on-index flattens and accuracy is a step function with zero slope. Only the first gives descent something to follow everywhere. (Chance loss over 10 classes is $\\ln 10\\approx2.30$.)`
        },
        symbols: [
    ...
    stages: [
      {
        ...
        symbols: [
          ...
          { sym: "AUC", desc: "area under the ROC (Receiver Operating Characteristic) curve — the probability the model ranks a random >50K earner above a random ≤50K one, independent of threshold." },
      ...
      },
      {
        ...
        data: {
          caption: "5-fold CV (Cross-Validation) fold-score table (AUC per held-out fold)",
    ...
    stages: [
      {
        ...
        symbols: [
          { sym: "knowledge base (KB)", desc: "the explicit collection of facts and MD-elicited (Medical Doctor–elicited) rules the engine reasons over (MYCIN's ~600 rules)." },
    ...
    stages: [
      {
        ...
        data: {
          ...
          note: `Ignoring the evidence (using the prior alone) would diagnose 'hypovolemia'. The posterior, conditioned on the readings, correctly elevates 'LV (left-ventricular) failure'.`
        },
        chart: {
      ...
      },
      {
        ...
        symbols: [
          { sym: "Gibbs sampling", desc: "an MCMC (Markov Chain Monte Carlo) method that resamples each variable in turn from its conditional given the others, producing samples from the posterior." },
  ...
  "scientific-modeling": {
    ...
    stages: [
      ...
      },
      {
        ...
        insight: `<b>The noise assumption picks the loss.</b> Repeated readings of one point scatter symmetrically (e.g. 4.01, 3.98, 4.03, 3.99) — textbook Gaussian — so squared loss is MLE (Maximum Likelihood Estimation) and a handful of normal-sized residuals each contribute fairly. But squared loss weights a residual by its SQUARE: a single 5σ outlier contributes 25× the pull of a 1σ point, so if the tails are actually fat you'd need a robust loss (Huber/SVR (Support Vector Regression)) instead. The data's noise shape, not habit, dictates the objective.`,
        data: {
  ...
  "linear-algebra-engines": {
    ...
    stages: [
      ...
      },
      {
        ...
        narrative: `<p>The engine must be fast at scale. The key insight: you only KEEP 28 of 784 components, so computing all 784 is wasted work. A randomized truncated SVD computes just the top-$k$ directions — it projects $X$ onto a small random subspace, captures the dominant directions there, and recovers the leading components at near-exact accuracy for a fraction of the cost. Build it on batched, BLAS-optimized (Basic Linear Algebra Subprograms) matrix multiplies (the hardware's fastest kernels) rather than Python loops, and it screams.</p>`,
        ...
        steps: [
          { type: "decide", prompt: "Full SVD of a 70k x 784 matrix is too slow. How do you speed it up while staying accurate?",
            options: [
              ...
              { label: "Loop over entries in pure Python", best: false, feedback: "elementwise Python loops abandon BLAS — the hardware-tuned matrix-multiply kernels that exploit caching and SIMD (Single Instruction, Multiple Data) — so they run orders of magnitude slower (hours, not seconds) for the identical math. Worse, they don't even reduce the work; they just do the same operations on a far slower engine. The whole game at this scale is staying inside optimized linear-algebra kernels, which Python loops explicitly leave." }
            ] },
