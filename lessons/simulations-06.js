  ...
  "ml-optimization-engine": {
    ...
    stages: [
      ...
      },
      {
        ...
        insight: `<b>Variance compounds with depth.</b> If each layer multiplies the signal's standard deviation by a factor $g$, then after 20 layers the scale is $g^{20}$. At $g=1.3$ that's $\\sim$190× (explode); at $g=0.8$ it's $\\sim$0.012× (vanish); only $g\\approx 1$ survives. He init for ReLU picks variance $2/\\text{fan-in}$ precisely to make $g\\approx 1$ — so a 512-wide layer draws weights with $\\sigma=\\sqrt{2/512}\\approx 0.0625$, not $0.5$ and not $10$.`,
        data: {
      ...
      },
      {
        ...
        narrative: `<p>With a sane learning rate, choose how to take steps. Plain full-batch descent is slow; you want momentum, which averages recent gradients to power through flat regions and dampen oscillation, and adaptivity, which gives each parameter its own effective step based on its gradient history. On top of that, a schedule anneals the global rate over time — a brief warmup to avoid early instability, then a smooth decay so the model takes big strides early and fine ones near the minimum.</p>`,
        concepts: ["dl-optimizers", "ai-sgd", "fnd-chain"],
        insight: `<b>Adaptivity + schedule earns its keep.</b> On this run, plain SGD reaches valid loss 0.49 in 60 epochs; Adam with warmup+cosine reaches 0.41 in 30 — fewer epochs AND a better minimum. The warmup matters too: skipping it and starting at the full $\\alpha=3\\!\\times\\!10^{-4}$ spikes the loss in the first 200 steps before settling, while a 500-step linear warmup is monotone from the start.`,
        data: {
        ...
        symbols: [
          { sym: "momentum", desc: "a running average of recent gradients used as the step direction; smooths noise and carries the optimizer across flat spots." },
          { sym: "$\\alpha$", desc: "the global learning rate, here varied over time by the schedule." },
          { sym: "warmup", desc: "a short opening phase that ramps $\\alpha$ from near-0 up to its peak to avoid early instability." },
          { sym: "cosine decay", desc: "annealing $\\alpha$ down a half-cosine curve so steps shrink smoothly as training converges." }
        ],
        steps: [{
          type: "decide", prompt: "Which optimizer + schedule for a noisy deep model?",
          options: [
            { label: "Adam with a warmup then cosine-decay learning-rate schedule", best: true, feedback: "design decision: combine adaptivity, momentum, and an annealing schedule. Mechanism: Adam divides each parameter's step by a running estimate of its gradient magnitude, so steep and flat directions get appropriately-sized moves despite messy mini-batch noise; momentum averages out that noise; warmup avoids the early-step spike (see the table) and cosine decay shrinks steps as you near the minimum, locking in the last fraction of loss. Tradeoff: a few more hyperparameters, but the defaults are robust. Best fit for a noisy deep model." },
  ...
  "model-evaluation": {
    ...
    stages: [
      ...
      },
      {
        ...
        narrative: `<p>Now tune. Search the hyperparameter space using the CV estimate as the objective — but search efficiently, since each trial costs a full K-fold fit. With ~6 hyperparameters, an exhaustive grid explodes combinatorially and wastes most of its budget on dimensions that don't matter. Random search covers the important dimensions far better for the same number of trials, and Bayesian search goes further by using past trials to propose promising next points. Crucially, every trial is scored by CV (never the test set) so the selection stays honest.</p>`,
        concepts: ["mlx-model-selection", "ml-regularization", "mlx-cross-validation"],
        insight: `<b>Random beats grid at equal budget.</b> A grid of just 4 values across 6 hyperparameters is $4^6=4096$ trials; 60 random trials cover the high-impact dimensions better than a coarse grid could. Here trial 41 (depth 8, lr 0.03, l2 2.0) wins at CV AUC 0.857 ± 0.005, beating the 0.78 baseline by +0.077. Note trial 58 (depth 12) scores LOWER at 0.831 with high variance — deeper isn't better; it overfits.`,
        data: {
  ...
  "expert-systems-reasoning": {
    ...
    stages: [
      ...
      },
      {
        ...
        narrative: `<p>With facts and rules in the base, you derive conclusions in one of two directions. Forward chaining is DATA-driven: start from known facts and fire every rule whose premises hold, deriving all consequences until nothing new appears (the closure). Backward chaining is GOAL-driven: start from the query you want to prove and expand only the rules that could establish it, recursing on their premises. For a single targeted query, backward chaining touches a tiny relevant subset; forward chaining computes everything.</p>`,
        ...
        symbols: [
          { sym: "forward chaining", desc: "data-driven inference: fire every applicable rule from the facts forward until the fact set stops growing (the closure)." },
          { sym: "backward chaining", desc: "goal-driven inference: start from the query and expand only rules that could prove it, recursing on premises." },
        ...
        steps: [{
          ...
          options: [
            { label: "Backward chaining from the goal, expanding only rules that could prove it", best: true, feedback: "design decision: a single targeted query is a goal-driven search, so chain backward. Mechanism: starting from the goal, the engine expands only rules whose conclusion matches it, then recurses on those rules' premises — touching just the ~6 rules on the proof path and stopping the instant the goal is proved. Tradeoff: backward chaining re-derives sub-goals if you ask many different queries, but for one question it's dramatically cheaper than computing the whole closure." },
            { label: "Forward chaining every rule to exhaustion", best: false, feedback: "forward chaining computes the ENTIRE closure — it fires all 412 rules and derives every consequence of the facts, the vast majority irrelevant to your one liftgate question (hazmat status, customs, routing, ...). You get the right answer buried in hundreds of unused derivations. Forward chaining shines when you need MANY conclusions at once; for a single goal it's wasteful." },
  ...
  "probabilistic-diagnosis": {
    ...
    stages: [
      ...
      },
      {
        ...
        narrative: `<p>Given observed symptoms, compute the posterior over causes. The brute-force way — enumerate the full joint distribution and sum — costs $2^9=512$ terms here and grows exponentially. Variable elimination instead sums out the unobserved variables one at a time in a good order, reusing intermediate factors so it never materializes the full joint. The elimination order matters: a smart order keeps the intermediate factors small, giving exact posteriors orders of magnitude faster than enumeration.</p>`,
        ...
        steps: [{
          ...
          options: [
            { label: "Variable elimination, marginalizing out unobserved nodes in a good order", best: true, feedback: "design decision: for a moderate network needing exact answers, eliminate hidden variables smartly. Mechanism: VE sums out one unobserved node at a time, building small intermediate factors and reusing them, so it computes the exact posterior without ever forming the $2^9$ full joint — ~40 ops vs 512. Tradeoff: finding a good elimination order takes thought (a bad order can blow the factors back up), but for a moderate net it's the right exact method." },
    ...
    stages: [
      ...
      },
      {
        ...
        symbols: [
          ...
          { sym: "residual std", desc: "standard deviation of the leftover residuals; the noise scale the model couldn't explain." },
      ...
      },
      {
        ...
        narrative: `<p>A fit without error bars is half an answer — a scientific claim needs to say how sure it is. Two kinds of uncertainty matter: credible intervals on the parameters $a,b$ (how tightly the data constrains them), and predictive bands on the rate (how uncertain a future measurement is). A proper probabilistic fit — Bayesian regression or a Gaussian process — delivers both, and critically its predictive band WIDENS where data is thin, so an extrapolated prediction honestly advertises its lower confidence instead of pretending to know.</p>`,
        ...
        symbols: [
          ...
          { sym: "predictive band", desc: "the uncertainty range around a predicted rate; widens where data is sparse." },
  ...
  "linear-algebra-engines": {
    ...
    stages: [
      ...
      },
      {
        ...
        narrative: `<p>Run the SVD and decide how many components to keep. Each singular value $\\sigma_i$ contributes variance $\\sigma_i^2$, so the cumulative fraction $\\sum_{i\\le k}\\sigma_i^2 / \\sum_i \\sigma_i^2$ is exactly the variance retained by keeping the top $k$. The rank choice is a trade: more components mean higher fidelity but less compression. You pick the SMALLEST $k$ that meets your variance target — a classic elbow on the cumulative-variance curve.</p>`,
