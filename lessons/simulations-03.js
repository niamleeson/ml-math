...
window.SIMULATIONS = Object.assign(window.SIMULATIONS || {}, {
  "llm-chatbots": {
    title: "LLM (Large Language Model) Chatbot Assistant",
    icon: "💬",
    ...
    stages: [
      ...
      },
      {
        ...
        narrative: `<p>A 7B model generating tokens one at a time has real latency and GPU (Graphics Processing Unit) cost, because each new token is a full forward pass over the network. Three serving tricks cut both: <b>quantization</b> stores weights in 8-bit instead of 16/32-bit to shrink memory and speed matmuls; the <b>KV-cache (Key-Value cache)</b> saves the keys/values of past tokens so attention never recomputes them; and <b>continuous batching</b> packs many users' requests through the GPU together. You stream tokens to hide latency and roll out behind a canary to limit blast radius.</p>`,
        ...
        symbols: [
          ...
          { sym: "p50 / p99", desc: "median and 99th-percentile latency; p99 is the tail users feel on a bad request, the number you SLO (Service Level Objective) against." },
  ...
  "language-understanding": {
    title: "Text Mining & NLP (Natural Language Processing) Analytics",
    icon: "📰",
    ...
    stages: [
      ...
      },
      {
        ...
        narrative: `<p>You need two kinds of labels, and two public datasets supply them. <b>Intent labels</b> come from <b>banking77</b> — <b>13,083</b> queries already labeled into 77 intents (10,003 train / 3,080 test). <b>NER labels</b> come from <b>CoNLL-2003</b>, the standard span-labeled benchmark: every token carries a BIO (Begin-Inside-Outside) tag for one of four entity types — <b>PER, ORG, LOC, MISC</b> — and it's expensive to make because annotators must mark exact span boundaries, which is why CoNLL ships with measured inter-annotator agreement. The two label types have completely different cost and quality profiles, which shapes how much of each you collect.</p>`,
      ...
      },
      {
        ...
        narrative: `<p>Look before you model. <b>LDA</b> (latent Dirichlet allocation) is an unsupervised topic model that discovers clusters of co-occurring words — themes — without using your labels, so it can surface routes your schema forgot; it's the classic tool run on topic corpora like <b>20 Newsgroups</b> and <b>AG News</b>. Meanwhile <b>t-SNE</b> (t-distributed Stochastic Neighbor Embedding) projects the high-dimensional embeddings to 2-D so you can see which classes overlap. With 77 banking77 intents, class balance and any non-English queries shape later choices, so you profile all of it first.</p>`,
      ...
      },
      {
        ...
        narrative: `<p>Models need vectors, not strings. <b>TF-IDF</b> (Term Frequency–Inverse Document Frequency) turns a ticket into a sparse vector that weights each word by how often it appears here versus everywhere — a fast, strong baseline. <b>Contextual embeddings</b> from a transformer instead give each word a dense vector that depends on its neighbors, so 'crash' near 'payment' and 'crash' near 'app' get different representations. The plan is baseline first, then upgrade where it pays off.</p>`,
  ...
  "speech-translation": {
    ...
    stages: [
      {
        phase: "Frame", icon: "🎯", title: "Frame the problem",
        narrative: `<p>You're building speech-to-text plus translation for a meetings app, a two-stage pipeline: ASR (Automatic Speech Recognition; audio → English text) then MT (Machine Translation; English → German). Each half has a standard benchmark and metric: ASR is measured by <b>WER</b> (Word Error Rate) on sets like <b>LibriSpeech</b>, and MT by <b>BLEU</b> (Bilingual Evaluation Understudy) on the <b>WMT</b> (Workshop on Machine Translation) news-translation EN-DE (English–German) task. <b>WER</b> (word error rate) counts how many word edits — insertions, deletions, substitutions — turn the transcript into the reference, divided by reference length; lower is better. <b>BLEU</b> scores how much the translation's n-grams overlap a reference translation; higher is better. Picking the right metric per task is the framing decision.</p>`,
      ...
      },
      {
        ...
        narrative: `<p>ASR maps a variable-length spectrogram (~720 frames) to a much shorter word sequence (~20 words), and the alignment between them is unknown. Three ways to bridge that gap: <b>CTC</b> (Connectionist Temporal Classification) sums over all possible frame-to-token alignments so you need no hand-built alignment; a classic <b>HMM</b> (Hidden Markov Model) models phoneme states with explicit transition probabilities; or an attention <b>seq2seq</b> (sequence-to-sequence) encoder-decoder learns, for each output word, which input frames to look at. MT is a clean transformer seq2seq.</p>`,
        concepts: ["dl-attention", "mod-transformer", "dl-lstm-gru", "ai-hmm"],
        insight: `<b>The core problem is alignment: 720 frames → ~20 words.</b> Attention solves it by computing, for each output token, a softmax distribution over all input frames — effectively learning the alignment instead of hand-building it. CTC solves the same mismatch differently: it lets each of the 720 frames emit either a character or a special <b>blank</b> $\\varnothing$, then defines a collapse rule — <i>merge consecutive duplicate labels, then delete blanks</i> — that maps a 720-long frame path down to the ~20-character word sequence (e.g. <i>c c $\\varnothing$ a a t</i> → <i>cat</i>). Many frame paths collapse to the same text, so the CTC loss is $-\\log$ of the <i>summed</i> probability over <b>all</b> paths that collapse to the target (computed efficiently by dynamic programming). Both beat the classic HMM-GMM (Gaussian Mixture Model) pipeline substantially on WER, and transformers are state-of-the-art for both halves, so one architecture family covers ASR and MT.`,
        data: {
  ...
  "image-generation": {
    ...
    stages: [
      ...
      },
      {
        ...
        insight: `<b>Caption source quality varies widely.</b> You start from a LAION-style web set: of <b>18.0M</b> pairs the captions are mostly <b>alt-text</b> (<b>60%</b>, noisy/SEO-spammy — this is what raw LAION is), with humans (<b>25%</b>, clean but expensive) and auto-captioning (<b>15%</b>) added for your product domain. The human 25% teaches prompt alignment best; the alt-text 60% is volume with mixed accuracy. Resolution is a hard ceiling — <b>91%</b> are ≥512px (LAION ships a high-resolution subset for exactly this reason) because you cannot generate detail finer than the training images. A safety filter removed 240K NSFW (Not Safe For Work)/violent images.`,
        data: {
      ...
      },
      {
        ...
        narrative: `<p>Three families of generator, each with a different training dynamic. <b>Diffusion</b> learns to reverse a noising process — it denoises pure noise into an image over many small steps — giving stable training and strong quality. A <b>GAN</b> (Generative Adversarial Network) pits a generator against a discriminator in a minimax game; fast to sample but unstable (mode collapse). A <b>normalizing flow</b> uses invertible transforms for exact likelihoods but trails on image quality at scale. The choice shapes every later failure mode.</p>`,
      ...
      },
      {
        ...
        narrative: `<p>Diffusion's cost is its many denoising steps — each step is a full U-Net pass, so 1,000 steps is 1,000 passes per image. Two levers cut this: <b>fast samplers</b> reach good images in ~20 steps instead of 1,000, and <b>distillation</b> trains a student to take even bigger jumps. Then, because generation can produce harmful content, you add <b>safety filters</b> on both the input prompt and the output image. Batch on GPU (Graphics Processing Unit) and canary first.</p>`,
