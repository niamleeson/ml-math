  ...
  window.LESSONS.push({
    ...
    sections: [
      ...
      { h: "One network, every noise level: why it must be told t",
        ...
        <p>Notice the network's signature: $\\epsilon_\\theta(x_t, t)$. It takes <b>two</b> inputs — the noisy point <b>and the timestep</b>. This is not decoration. The same noisy point means completely different things at different noise levels. Concretely: suppose the network sees the point $(2.0,\\ 1.9)$, which sits between two clusters, on the ring but not on any mode.</p>
        <ul>
          ...
          <li>If $t = 190$, the point is mostly noise. Its clean ancestor was almost certainly on one of the clusters, and the best guess is a <b>large</b> noise vector — the displacement that would carry a cluster point out to $(2.0,\\ 1.9)$, scaled the way part 7's posterior dictates.</li>
        </ul>
      ...
      { h: "Reading the loss curve",
        ...
        <ul>
          ...
          <li><b>It plateaus well above zero — and that is correct.</b> Part 9's loss-versus-$t$ curve showed that even the exact best predictor has leftover error: the variance of $\\epsilon$ that $x_t$ cannot reveal. At tiny $t$ the added noise is microscopic next to the data's own spread, so the noisy point tells you almost nothing about which $\\epsilon$ was drawn, and the floor there is near 1 no matter how good the network is. Averaged over all $t$, the floor for our dataset sits around 0.2-0.3. A training loss near zero would be evidence of a bug (or of the network memorizing specific noise draws) — not of success.</li>
        </ul>
        <p>In the canonical notebook run, the loss starts at about 1.0, drops below 0.5 within the first few hundred steps, and flattens around 0.25 by step 2,500 — falling, noisy, floored above zero. Exactly the healthy shape.</p>` },
      ...
      { h: "Did it work? Checks and failure modes",
        ...
        <table class="symbols">
          ...
          <tr><td>Loss spikes wildly or becomes NaN within a few hundred steps</td><td>Learning rate too high — steps overshoot the downhill direction</td><td>Lower $\\eta$ (we use $10^{-3}$); if you multiply it by 100, expect exactly this</td></tr>
          <tr><td>Loss looks healthy, but samples huddle far inside the ring at the wrong radius</td><td>$T$ too small — the forward process never reaches anything like pure noise, so starting Algorithm 2 from $\\mathcal{N}(0, \\mathbf{I})$ starts it somewhere the network never trained</td><td>Raise $T$ (and/or the noise doses) so $\\bar\\alpha_T$ is small</td></tr>
      ...
      { h: "Sanity checks",
        body: `<p>Every one of these is verified with an assert in the notebook.</p>
        <ul>
          ...
          <li><b>The floor is real.</b> Final training loss settles near 0.25, not 0 — matching part 9's irreducible-variance analysis, averaged over $t$.</li>
          <li><b>Last step adds no noise.</b> With $z = 0$ at $t = 1$, the final positions are posterior means, not draws — remove that special case and the clusters get a visible extra blur of size $\\sqrt{\\beta_1} = 0.01$; tiny here, but the principle (never hand the user a gratuitously noisy final answer) matters more at scale.</li>
        </ul>` },
    ...
    practice: [
      ...
      { q: "Failure-mode matching. Four runs went wrong. Match each symptom to its cause (learning rate too high; $T$ too small; time embedding missing; sampling noise $\\sigma_t z$ dropped) and state the fix. (A) Samples form eight needle-thin dots with within-cluster spread near 0.01. (B) The loss becomes NaN before step 500. (C) The loss plateaus noticeably higher than usual and samples form one smeared ring with no distinct clusters. (D) The loss curve looks healthy but samples huddle at mean radius about 1.5 instead of 4.",
        steps: [
          ...
          { do: "(B) NaN loss early in training is the signature of a learning rate too high. Fix: reduce $\\eta$ toward $10^{-3}$.", why: "Oversized steps overshoot the downhill direction, the loss grows, the slopes grow with it, and the feedback loop blows the weights up to overflow within a few hundred steps." },
          ...
          { do: "(D) Healthy loss but samples at radius 1.5 is $T$ too small. Fix: increase $T$ so $\\bar\\alpha_T$ is genuinely tiny.", why: "With too few steps the forward process never reaches pure noise, so $q(x_T)$ still carries signal — but Algorithm 2 starts from $\\mathcal{N}(0, \\mathbf{I})$ anyway, a mismatch the training loss never measures. The walk begins off-distribution and never recovers the full radius." }
        ],
        answer: "A: dropped $\\sigma_t z$ — restore the per-step noise. B: learning rate too high — lower $\\eta$. C: missing time embedding — restore the $t$ input. D: $T$ too small — lengthen the schedule so $\\bar\\alpha_T \\approx 0$. Note that only B shows up in the loss curve; A, C, D require looking at samples — which is why we always evaluate generations, not just losses." }
    ]
  });
})();
