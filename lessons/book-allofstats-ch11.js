...
(function () {
  ...
  // Chart: prior vs posterior for the Bernoulli example (lesson 1).
  // Beta(s+1, n-s+1) with the book's setup; illustrative n=10, s=7 (flat Beta(1,1) prior).
  (function () {
    ...
    for (var k = 1; k <= 19; k++) {
      ...
      post.push(Number(betaPdf(x, 8, 4).toFixed(3)));         // Beta(s+1,n-s+1)=Beta(8,4), n=10,s=7
    }
    window.CODEVIZ["aos-ch11-bayesian-method"] = {
      charts: [{
        type: "line",
        title: "Prior vs posterior for p (Bernoulli, flat prior, n=10, s=7)",
        interpret: "The flat Beta(1,1) prior is constant; after observing 7 successes in 10 the posterior Beta(8,4) concentrates near p=0.7 (the MLE), pulled slightly toward the prior mean 0.5 — Bayes estimate (s+1)/(n+2)=8/12=0.667.",
        xlabel: "p",
        ylabel: "density",
        series: [
          { name: "prior Beta(1,1)", color: "#7ee787", points: xs.map(function (x, i) { return [Number(x), prior[i]]; }) },
          { name: "posterior Beta(8,4)", color: "#4ea1ff", points: xs.map(function (x, i) { return [Number(x), post[i]]; }) }
        ]
      }]
    };
  })();
