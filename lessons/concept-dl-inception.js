(function () {
  window.LESSONS.push({
    ...
    bigIdea:
      `<p>A normal convolution forces you to pick one filter size — a filter is a small window of weights slid across the image, and its size sets how big a patch it looks at. Small filters see fine detail; large filters see bigger structures. Which is right? You usually do not know.</p>
       <p><b>Analogy — bring every lens.</b> A photographer unsure whether a scene needs a wide-angle, normal, or zoom lens could keep swapping lenses and guessing. The Inception module instead mounts all the lenses at once, shoots the same scene through each, and hands the next stage every version — letting it decide which views were useful.</p>
       <p>The <b>Inception module</b> sidesteps the choice: it runs several convolutions <b>in parallel</b> on the same input — a 1×1, a 3×3, a 5×5, and a pooling branch — then <b>concatenates</b> all their outputs along the channel axis (stacks them as extra feature maps, so nothing is averaged away). The next layer sees features at every scale and the network learns which to use.</p>
       <p>The catch: a 5×5 conv over many input channels is very expensive. The fix is the <b>1×1 convolution</b>: a tiny, cheap conv that mixes and <b>reduces</b> the number of channels first (think of it as summarizing 192 feature maps down to 16 before the costly work), so the expensive 3×3 and 5×5 convs run on a much thinner input.</p>`,
    buildup:
