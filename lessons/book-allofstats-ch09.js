...
(function () {
  ...
  window.CODEVIZ["aos-ch9-maximum-likelihood"] = {
    charts: [
      {
        type: "line",
        title: "Bernoulli likelihood for n=20 and S=12 (Figure 9.1)",
        interpret: "The likelihood is proportional to p^12(1-p)^8 and reaches its maximum at p-hat = S/n = 12/20 = 0.6.",
        xlabel: "p",
        ylabel: "likelihood / max",
        series: [{ name: "relative likelihood", color: "#4ea1ff", points: [[0.1,0.0000003],[0.2,0.000482],[0.3,0.021476],[0.4,0.197531],[0.5,0.668506],[0.6,1.0],[0.7,0.636578],[0.8,0.123318],[0.9,0.00198]] }]
      },
      {
        type: "line",
        title: "Uniform(0, theta) likelihood shape (Figure 9.2)",
        interpret: "The likelihood is 0 below the sample maximum X_(n), jumps at X_(n), then decreases like theta^{-n}; the MLE is X_(n).",
        xlabel: "theta / X_(n)",
        ylabel: "relative likelihood",
        series: [{ name: "relative likelihood", color: "#ffb454", points: [[0.6,0],[0.9,0],[1.0,1.0],[1.2,0.401878],[1.5,0.087792],[2.0,0.015625]] }]
      }
    ],
    code: "import numpy as np\n\n# Bernoulli likelihood from Figure 9.1: n = 20, S = 12\np = np.linspace(0.01, 0.99, 99)\nL = p**12 * (1-p)**8\np_hat = p[np.argmax(L)]\nprint(round(p_hat, 2))        # 0.60 = 12/20\n\n# Uniform(0, theta): L(theta)=0 if theta < X_(n), else theta^{-n}\nx = np.array([0.23, 0.54, 0.70, 0.31])\ntheta_hat = x.max()\nprint(theta_hat)              # X_(n), the book's MLE"
  };
  ...
  window.CODEVIZ["aos-ch9-optimality"] = {
    ...
    code: "import numpy as np\n\n# Normal mean (MLE): asymptotic variance factor sigma^2\n# Normal median: asymptotic variance factor sigma^2*pi/2\nsigma2 = 1.0\nmean_factor = sigma2\nmedian_factor = sigma2 * np.pi / 2\nare = mean_factor / median_factor\nprint(round(median_factor, 4))  # 1.5708\nprint(round(are, 4))            # 0.6366 = 2/pi"
  };
  ...
  B({
    ...
    sections: [
      ...
      { h: "Worked example: Bernoulli information", body: "<p>For $X_1,\\dots,X_n \\sim \\text{Bernoulli}(p)$ (Example 9.20):</p><ul class='steps'><li>Score: $s(X;p) = X/p - (1-X)/(1-p)$.</li><li>Negative derivative: $-s'(X;p) = X/p^2 + (1-X)/(1-p)^2$.</li><li>Take the expectation (using $\\mathbb{E}X = p$): $I(p) = p/p^2 + (1-p)/(1-p)^2 = 1/(p(1-p))$.</li><li>Standard error: $\\widehat{\\text{se}} = \\sqrt{1/(nI(\\hat{p}_n))} = \\sqrt{\\hat{p}_n(1-\\hat{p}_n)/n}$.</li></ul><p>For Poisson$(\\lambda)$ (Example 9.22), $I_1(\\lambda) = 1/\\lambda$, the MLE is $\\hat{\\lambda}_n = \\bar{X}_n$, and $\\widehat{\\text{se}} = \\sqrt{\\hat{\\lambda}_n/n}$.</p><pre><code class=\"language-python\">import math\n\n# Bernoulli information using Figure 9.1 numbers: n=20, S=12.\nn, S = 20, 12\np_hat = S / n                         # 0.60\nse_p = math.sqrt(p_hat * (1-p_hat) / n)\nprint(round(se_p, 4))                 # 0.1095</code></pre>" },
      { h: "The delta method", body: "<p>If $\\tau = g(\\theta)$ for a smooth $g$, its MLE is $\\hat{\\tau} = g(\\hat{\\theta})$ by equivariance. The <strong>delta method</strong> (Theorem 9.24) gives its standard error: if $g$ is differentiable with $g'(\\theta) \\neq 0$, then $(\\hat{\\tau}_n - \\tau)/\\widehat{\\text{se}}(\\hat{\\tau}) \\rightsquigarrow N(0,1)$ where $\\widehat{\\text{se}}(\\hat{\\tau}_n) = |g'(\\hat{\\theta})|\\,\\widehat{\\text{se}}(\\hat{\\theta}_n)$. So you just multiply the parameter's standard error by the absolute slope of $g$. Example 9.25: for Bernoulli with $\\psi = \\log(p/(1-p))$, $g'(p) = 1/(p(1-p))$, giving $\\widehat{\\text{se}}(\\hat{\\psi}_n) = 1/\\sqrt{n\\hat{p}_n(1-\\hat{p}_n)}$. Example 9.26 gives $\\widehat{\\text{se}}(\\log\\hat{\\sigma})=1/\\sqrt{2n}$.</p><pre><code class=\"language-python\">import math\n\n# Delta method for Bernoulli log-odds, reusing n=20, S=12.\nn, p_hat = 20, 12/20\npsi_hat = math.log(p_hat / (1-p_hat))\nse_psi = 1 / math.sqrt(n * p_hat * (1-p_hat))\nprint(round(psi_hat, 4))  # 0.4055\nprint(round(se_psi, 4))   # 0.4564\nprint(round(1/math.sqrt(2*n), 4))  # 0.1581 for log(sigma)</code></pre>" },
  ...
  B({
    ...
    sections: [
      ...
      { h: "Minimal sufficiency", body: "<p>A statistic $T$ is <strong>minimal sufficient</strong> (Definition 9.35) if it is sufficient and is a function of every other sufficient statistic — the most concise summary. Theorem 9.36: $T$ is minimal sufficient if and only if $T(x^n) = T(y^n)$ exactly when $x^n \\leftrightarrow y^n$. A sufficient statistic induces a partition of the outcome space, and sufficiency can be viewed in terms of these partitions. Example 9.37 illustrates this with two Bernoulli$(\\theta)$ draws:</p><table class='extable'><thead><tr><th>$X_1$</th><th>$X_2$</th><th>$V=X_1$</th><th>$T=X_1+X_2$</th><th>$U=(T,X_1)$</th></tr></thead><tbody><tr><td class='num'>0</td><td class='num'>0</td><td class='num'>0</td><td class='num'>0</td><td class='row-h'>(0,0)</td></tr><tr><td class='num'>0</td><td class='num'>1</td><td class='num'>0</td><td class='num'>1</td><td class='row-h'>(1,0)</td></tr><tr><td class='num'>1</td><td class='num'>0</td><td class='num'>1</td><td class='num'>1</td><td class='row-h'>(1,1)</td></tr><tr><td class='num'>1</td><td class='num'>1</td><td class='num'>1</td><td class='num'>2</td><td class='row-h'>(2,1)</td></tr></tbody></table><p>The induced partitions make $V$ not sufficient, $T$ and $U$ sufficient, and $T$ minimal sufficient. Rao-Blackwell then improves $\\hat{\\theta}=X_1$ to $\\tilde{\\theta}=\\mathbb{E}(X_1\\mid T)=(X_1+X_2)/2$.</p>" }
    ],
    takeaways: [
