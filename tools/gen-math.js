...
function scaffold(topic, title, i, prevId) {
  ...
  const note = (txt) => `<p class="scaffold-note">${txt}</p>`;
  const sections = [
    { h: "Connections", body:
        `<p>Where this sits in <b>${topic.name}</b> (${topic.cat}):</p>` +
        `<ul class="steps">` +
        `<li><b>Builds on</b> — ${i === 0 ? "the prerequisites for this topic" : "the previous lesson, <i>" + cleanTitle(topic.lessons[i - 1]) + "</i>"}.</li>` +
        `<li><b>Leads to</b> — ${i === topic.lessons.length - 1 ? "the next topic in the track" : "the next lesson, <i>" + cleanTitle(topic.lessons[i + 1]) + "</i>"}.</li>` +
        `<li><b>Used with</b> — the other concepts in ${topic.name} and its capstone.</li>` +
        `</ul>` },
    { h: "Motivation & Intuition", body: `<p><i>${tagline}</i></p>` + note("Warm motivation and the core mental model — to be authored in the next pass.") },
    { h: "Definition & Assumptions", body: note("Precise definition, every symbol in plain English, and the conditions under which it holds — to be authored.") },
    { h: "Worked Example & Practice", body: note("A one-operation-per-step worked example with real numbers, plus faded practice — to be authored.") },
    { h: "Real-World Applications in CS & ML", body: note("Concrete uses in computer science and machine learning, each with live numbers — to be authored.") }
  ];
  const takeaways = [ `<b>${T}</b> — a single concept within ${topic.name}.`, "Scaffold lesson: structure is final; full content is being authored." ];
  return { id: lessonId(topic, i), title: T, tagline, sections, takeaways, prereqs: prevId ? [prevId] : undefined };
}
