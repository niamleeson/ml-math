...
1. Define the five components of a deterministic search problem for a robot moving on a grid.
2. Which data structure implements BFS, DFS, and UCS respectively?
3. In an unweighted graph, why does BFS return a shortest path in number of edges?
4. For UCS, if a frontier node has past cost 7 and an outgoing edge of cost 4, what priority is assigned to the successor through that edge?
5. On a four-neighbor unit-cost grid, is Manhattan distance to the goal admissible? Why?

### 🔴 Hard

1. Prove the UCS pop invariant for nonnegative edge costs.
2. Construct a consistent heuristic by relaxing a grid problem that originally forbids diagonal motion.
3. Prove that consistency implies admissibility when $h(s_{\text{end}})=0$.
4. Explain why adding a constant to every negative edge cost can change the optimal path.
5. Design a weighted graph where an inadmissible heuristic causes A* to return a nonoptimal path.

<details><summary>Solutions</summary>

### 🟢 Easy Solutions

1. A grid search problem has $s_{\text{start}}$ equal to the robot's initial cell; $\operatorname{Actions}(s)$ equal to legal moves such as up, down, left, and right; $\operatorname{Cost}(s,a)$ equal to the cost of taking a move, often 1 or a terrain cost; $\operatorname{Succ}(s,a)$ equal to the cell reached after the move; and $\operatorname{IsEnd}(s)$ true exactly when the robot is at the goal cell.

2. BFS uses a FIFO queue, DFS uses a LIFO stack or recursion, and UCS uses a minimum-priority queue keyed by $\operatorname{PastCost}(s)$.

3. BFS expands all states at depth 0, then all states at depth 1, then all states at depth 2, and so on. Therefore, the first time BFS reaches or pops a goal in an unweighted graph, no path with fewer edges can still be undiscovered.

4. The successor receives candidate priority $7+4=11$ because UCS prioritizes the total known past cost of the path to that successor.

5. Yes. Manhattan distance counts the minimum number of horizontal and vertical moves needed if there were no walls. Walls can only make the real path longer or equal, never shorter, so Manhattan distance never overestimates the true future cost.

### 🔴 Hard Solutions

1. Suppose UCS pops state $s$ with priority $g(s)$. If a cheaper path to $s$ existed, let $x$ be the first state on that cheaper path that had not been explored before $s$ was popped, and let $p$ be its predecessor on that path. The predecessor $p$ was explored earlier, so UCS would have inserted or updated $x$ with a cost no larger than the cheaper path's prefix cost. Because all remaining edge costs are nonnegative, that prefix cost is no larger than the cheaper full cost to $s$, which is less than $g(s)$. Thus some frontier priority would have been smaller than $g(s)$, contradicting that UCS popped $s$ next. Therefore the popped priority is optimal.

2. Start with a four-neighbor grid where real actions are up, down, left, and right with unit cost. Relax the problem by also allowing diagonal moves with unit cost and by ignoring walls if desired. The relaxed future cost has closed form $h(s)=\max(|r_s-r_g|,|c_s-c_g|)$ when diagonal unit moves are allowed. Because the relaxed problem removes constraints and never increases action costs, its optimal future cost is no larger than the real future cost, giving an admissible heuristic; as a relaxed future-cost heuristic, it is consistent.

3. Let $s=s_0,s_1,\dots,s_k=s_{\text{end}}$ be any path from $s$ to an end state. Consistency gives $h(s_i)\leq \operatorname{Cost}(s_i,a_i)+h(s_{i+1})$ for each step. Adding these inequalities telescopes to $h(s)\leq \sum_i \operatorname{Cost}(s_i,a_i)+h(s_{\text{end}})$. Since $h(s_{\text{end}})=0$, $h(s)$ is no larger than the cost of any path to the end. Therefore it is no larger than the minimum such path cost, so $h$ is admissible.

4. Adding a constant $C$ to every edge changes a path with $k$ edges by adding $kC$. Two paths with different numbers of edges therefore receive different total offsets. A path that was optimal because it used more negative or low-cost edges may become nonoptimal after the transformation, so the modified problem is not equivalent to the original problem.

5. Use nodes $S,A,B,G$ with edges $S\to A$ cost 1, $A\to G$ cost 4, $S\to B$ cost 2, and $B\to G$ cost 2. The optimal path is $S\to B\to G$ with cost 4. Define $h(A)=0$, $h(B)=100$, $h(G)=0$, and $h(S)=0$. A* pops $A$ before $B$, then pushes $G$ with cost 5 and priority 5; since $B$ has priority 102, A* pops $G$ and returns cost 5. The overestimate at $B$ is inadmissible and causes the failure.
