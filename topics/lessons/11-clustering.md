...
1. In one sentence, what does the k-means distortion $J(c,\mu)$ measure?
2. Why does standardizing features often matter before k-means?
3. On an elbow plot, why does inertia decrease as $k$ increases?
4. What does a silhouette value near $0$ mean for a data point?
5. Give one difference between k-means and hierarchical clustering.

### 🔴 Hard

1. Prove that one assignment step of k-means cannot increase the distortion when centroids are fixed.
2. Explain how the EM posterior $Q_i(z^{(i)})=P(z^{(i)}\mid x^{(i)};\theta)$ differs from a k-means assignment.
3. Construct a qualitative dataset where k-means++ can still converge to a bad local optimum.
4. For elongated chain-shaped clusters, which linkage rule might you try first and what risk does it carry?
5. If a point has $a=4$ and $b=6$, compute its silhouette coefficient and interpret it.

<details><summary>Solutions</summary>

### 🟢 Easy solutions

1. The distortion $J(c,\mu)=\sum_{i=1}^{m}\left\|x^{(i)}-\mu_{c^{(i)}}\right\|^2$ measures the total squared distance from every point to the centroid of the cluster it is assigned to.
2. Standardizing matters because Euclidean distance is unit-sensitive; a feature measured on a large numeric scale can dominate nearest-centroid assignments even if it is not more meaningful.
3. Inertia decreases as $k$ increases because extra centroids give the model more freedom: in the extreme, assigning every point its own centroid would drive distortion to zero.
4. A silhouette value near $0$ means the point is about as close to its own cluster as to the next nearest cluster, so it lies near a boundary or ambiguous region.
5. k-means directly optimizes centroid-based compact clusters for a chosen $k$, while hierarchical clustering builds a nested merge tree that can be cut at different levels.

### 🔴 Hard solutions

1. With fixed centroids $\mu_1,\ldots,\mu_k$, each point contributes $\left\|x^{(i)}-\mu_{c^{(i)}}\right\|^2$ to $J$. The assignment step sets $c^{(i)}=\operatorname*{argmin}_{j}\left\|x^{(i)}-\mu_j\right\|^2$, so each point's contribution is replaced by the smallest available squared distance. Replacing each term by a value no larger than before cannot increase the sum.
2. EM uses a distribution over latent assignments: $Q_i(z^{(i)})=P(z^{(i)}\mid x^{(i)};\theta)$. A boundary point can therefore partially belong to several components. k-means uses a hard assignment, placing each point in exactly one cluster by nearest centroid.
3. Imagine four dense sub-blobs arranged as two natural elongated groups, or one large diffuse cluster plus several tiny tight clusters. k-means++ improves initial spacing, but the k-means objective still favors spherical Voronoi cells, so it can split a large natural group or merge small neighboring groups if that lowers squared distance locally.
4. For elongated chain-shaped clusters, single linkage is a natural first try because it merges clusters by nearest pairwise connection and can follow non-convex shapes. Its risk is chaining: a thin bridge of noisy points can incorrectly connect two separate clusters.
5. The silhouette is $s=(b-a)/\max(a,b)=(6-4)/6=1/3\approx0.33$. The point is closer on average to its own cluster than to the nearest other cluster, but the separation is moderate rather than strong.
