...
1. For $w=(3,4)$ in a hard-margin SVM, what is the margin width $2/\|w\|$?
2. A point has label $y=-1$ and score $z=-0.2$. Compute its hinge loss.
3. In one sentence, what is a support vector?
4. If `C` is increased, do margin violations become cheaper or more expensive?
5. State the Gaussian/RBF kernel formula from the reference using $\sigma$.

### 🔴 Hard (5)

1. Derive why maximizing the margin width $2/\|w\|$ is equivalent to minimizing $\frac12\|w\|^2$.
2. For a soft-margin SVM, explain what $\xi_i=0$, $0<\xi_i<1$, and $\xi_i>1$ mean geometrically.
3. If an RBF SVM with high training accuracy has a very jagged boundary and poor test accuracy, should you usually increase or decrease `gamma`? Explain.
4. Use KKT complementary slackness to explain why only support vectors affect the dual prediction.
5. Design a tiny dataset where a linear SVM fails but an RBF SVM can succeed.
...
3. A support vector is a training point on or inside the margin whose Lagrange multiplier can be nonzero and whose position can affect the fitted SVM boundary.

4. Increasing `C` makes margin violations more expensive, so the SVM tries harder to avoid them.

5. The Gaussian/RBF kernel is
...
1. The margin width is

   $$
   \frac{2}{\|w\|}.
   $$

   Since $\|w\|>0$, maximizing $2/\|w\|$ is equivalent to minimizing $\|w\|$ because the reciprocal function decreases on positive inputs. The minimizer of $\|w\|$ is also the minimizer of $\|w\|^2$, because squaring is increasing on nonnegative values. Multiplying by $1/2$ does not change the optimizer. Hence
...
2. The soft-margin constraint is

   $$
   y_i(w^Tx_i-b)\ge1-\xi_i.
   $$

   - If $\xi_i=0$, then $y_i(w^Tx_i-b)\ge1$, so the point is correctly classified and on or outside the margin.
   - If $0<\xi_i<1$, then $0<1-\xi_i<1$, so the point is correctly classified but inside the margin.
   - If $\xi_i=1$, then the constraint permits $y_i(w^Tx_i-b)\ge0$, so the point may lie exactly on the decision boundary.
   - If $\xi_i>1$, then $1-\xi_i<0$, so the point may be misclassified.

3. You should usually decrease `gamma`. In the RBF kernel

   $$
   K(x,z)=\exp(-\gamma\|x-z\|^2),
   $$

   high `gamma` makes each point's influence very local. That can create a boundary that bends around individual training points, which is overfitting. Decreasing `gamma` increases the effective radius $\sigma=1/\sqrt{2\gamma}$ and smooths the boundary.

4. Complementary slackness says

   $$
   \alpha_i\left[y_i(w^Tx_i-b)-1\right]=0.
   $$

   If a point is strictly outside the margin, then

   $$
   y_i(w^Tx_i-b)>1,
   $$

   so the bracket is positive. The product can equal zero only if
   ...
   and kernel predictions use terms of the form

   $$
   \sum_i\alpha_i y_i K(x_i,x).
   $$

   Any point with $\alpha_i=0$ contributes nothing. Thus only support vectors, which have nonzero multipliers, affect the dual prediction.

5. One tiny example is the XOR pattern:
   ...
   No single line separates opposite corners of a square from the other opposite corners, so a linear SVM fails. An RBF SVM can succeed because it can form localized nonlinear regions around points of the same class.
