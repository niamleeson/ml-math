...
### ✍️ Toy 1 · Empirical risk is a sample mean

Generalization starts with the training average: add the per-example losses, then divide by the
number of examples.
...
t1_loss = np.array([0.10, 0.20, 0.15, 0.35, 0.40, 0.05])  # -> six per-example losses
t1_total = float(t1_loss.sum())                 # -> 1.25
t1_risk = float(t1_loss.mean())                 # -> 0.20833333333333334
print("losses:", t1_loss.tolist())             # -> [0.1, 0.2, 0.15, 0.35, 0.4, 0.05]
print("loss total:", round(t1_total, 3))        # -> 1.25
print("empirical risk:", round(t1_risk, 3))    # -> 0.208
assert abs(t1_risk - 1.25 / 6) < 1e-12

plt.figure(figsize=(4.6, 2.8))
plt.bar(range(t1_loss.size), t1_loss, color="steelblue")
plt.axhline(t1_risk, color="crimson", linestyle="--", label=f"mean={t1_risk:.3f}")
plt.xlabel("example")
plt.ylabel("loss")
plt.title("Toy 1 · empirical risk is the average")
plt.legend()
plt.show()
```
▶ What you'll see: six losses collapsed into one dashed mean near `0.208`.

### ✍️ Toy 2 · IID means train and future match

The identically-distributed part says the train and future mechanisms should have the same center;
a shifted future sample breaks that contract.

```python
import numpy as np                              # sample summaries.

t2_rng = np.random.default_rng(0)               # seeded local generator.
t2_train = np.array([1.8, 2.0, 2.1, 1.9, 2.2, 1.7, 2.3, 2.0])  # -> train sample
t2_future_iid = np.array([1.9, 2.1, 2.0, 2.2, 1.8, 2.1, 1.7, 2.2])  # -> same mechanism
t2_future_shift = np.array([2.8, 3.1, 3.0, 3.2, 2.9, 3.3, 3.1, 2.7])  # -> shifted mechanism
t2_train_mean = float(t2_train.mean())          # -> 2.0
t2_iid_mean = float(t2_future_iid.mean())       # -> 2.0
t2_shift_mean = float(t2_future_shift.mean())   # -> 3.0125
t2_iid_gap = abs(t2_train_mean - t2_iid_mean)   # -> 0.0
t2_shift_gap = abs(t2_train_mean - t2_shift_mean)  # -> 1.0125000000000002
print("means train/iid/shift:", [round(t2_train_mean, 3), round(t2_iid_mean, 3), round(t2_shift_mean, 3)])  # -> [2.0, 2.0, 3.013]
print("mean gaps iid/shift:", [round(t2_iid_gap, 3), round(t2_shift_gap, 3)])  # -> [0.0, 1.013]
assert t2_iid_gap < 0.05
assert t2_shift_gap > 1.0

plt.figure(figsize=(4.8, 2.8))
plt.bar(["train", "future iid", "future shifted"], [t2_train_mean, t2_iid_mean, t2_shift_mean], color=["teal", "orange", "crimson"])
plt.ylabel("sample mean")
plt.title("Toy 2 · shift moves the future mean")
plt.show()
```
▶ What you'll see: train and IID future means align at `2.0`, while the shifted future mean jumps to `3.013`.

### ✍️ Toy 3 · Validation can expose flexible overfit

A flexible polynomial may reduce training MSE, but a held-out validation sample checks whether that
extra freedom survives new IID rows.

```python
import numpy as np                              # polynomial fitting and MSE.

t3_rng = np.random.default_rng(0)               # seeded local generator.
t3_x_train = np.array([-1.0, -0.7, -0.4, -0.1, 0.2, 0.5, 0.8, 1.0])  # -> train inputs
t3_y_train = 1.0 + 2.0 * t3_x_train + np.array([0.45, -0.25, 0.20, -0.10, 0.15, -0.35, 0.30, -0.20])  # -> noisy train labels
t3_x_val = np.array([-0.9, -0.6, -0.3, 0.0, 0.3, 0.6, 0.9])  # -> validation inputs
t3_y_val = 1.0 + 2.0 * t3_x_val + np.array([0.05, -0.05, 0.00, 0.05, -0.05, 0.00, 0.05])  # -> validation labels
t3_coef_line = np.polyfit(t3_x_train, t3_y_train, 1)  # -> [1.872 1.03 ]
t3_coef_flex = np.polyfit(t3_x_train, t3_y_train, 5)  # -> [-3.506  0.591  4.137 -0.48   1.061  1.03 ]
t3_train_line = np.polyval(t3_coef_line, t3_x_train)  # -> line train predictions
t3_train_flex = np.polyval(t3_coef_flex, t3_x_train)  # -> flexible train predictions
t3_val_line = np.polyval(t3_coef_line, t3_x_val)      # -> line validation predictions
t3_val_flex = np.polyval(t3_coef_flex, t3_x_val)      # -> flexible validation predictions
t3_train_mse = np.array([np.mean((t3_y_train - t3_train_line) ** 2), np.mean((t3_y_train - t3_train_flex) ** 2)])  # -> [0.066 0.026]
t3_val_mse = np.array([np.mean((t3_y_val - t3_val_line) ** 2), np.mean((t3_y_val - t3_val_flex) ** 2)])  # -> [0.009 0.011]
print("train y:", np.round(t3_y_train, 2).tolist())     # -> [-0.55, -0.65, 0.4, 0.7, 1.55, 1.65, 2.9, 2.8]
print("validation y:", np.round(t3_y_val, 2).tolist())  # -> [-0.75, -0.25, 0.4, 1.05, 1.55, 2.2, 2.85]
print("line coefficients:", np.round(t3_coef_line, 3).tolist())  # -> [1.872, 1.03]
print("flex coefficients:", np.round(t3_coef_flex, 3).tolist())  # -> [-3.506, 0.591, 4.137, -0.48, 1.061, 1.03]
print("train MSE line/flex:", np.round(t3_train_mse, 3).tolist())  # -> [0.066, 0.026]
print("val MSE line/flex:", np.round(t3_val_mse, 3).tolist())      # -> [0.009, 0.011]
assert t3_train_mse[1] < t3_train_mse[0]
assert t3_val_mse[1] > t3_val_mse[0]

plt.figure(figsize=(5.0, 3.2))
t3_grid = np.linspace(-1.0, 1.0, 100)
plt.scatter(t3_x_train, t3_y_train, color="black", label="train")
plt.plot(t3_grid, np.polyval(t3_coef_line, t3_grid), color="teal", label="degree 1")
plt.plot(t3_grid, np.polyval(t3_coef_flex, t3_grid), color="crimson", label="degree 5")
plt.xlabel("x")
plt.ylabel("prediction")
plt.title("Toy 3 · flexibility fits train noise")
plt.legend()
plt.show()
```
▶ What you'll see: the flexible curve lowers train MSE but slightly loses to the line on validation.

### ✍️ Toy 4 · Complexity cost enters the full score

Adding a complexity cost puts raw fit and model burden on the same selection scale.

```python
import numpy as np                              # score vectors.

t4_rng = np.random.default_rng(0)               # seeded local generator.
t4_names = np.array(["line", "small poly", "wide poly", "lookup"])  # -> candidates
t4_risk = np.array([0.25, 0.21, 0.18, 0.16])    # -> empirical risks
t4_cost = np.array([0.03, 0.08, 0.13, 0.21])    # -> complexity costs
t4_score = t4_risk + t4_cost                    # -> [0.28 0.29 0.31 0.37]
t4_best = int(t4_score.argmin())                # -> 0
print("risks:", t4_risk.tolist())              # -> [0.25, 0.21, 0.18, 0.16]
print("costs:", t4_cost.tolist())              # -> [0.03, 0.08, 0.13, 0.21]
print("full scores:", np.round(t4_score, 3).tolist())  # -> [0.28, 0.29, 0.31, 0.37]
print("winner:", t4_names[t4_best])            # -> line
assert t4_names[t4_best] == "line"

plt.figure(figsize=(5.0, 2.8))
plt.bar(t4_names, t4_score, color=["seagreen", "gray", "gray", "gray"])
plt.ylabel("risk + cost")
plt.title("Toy 4 · cost-aware score prefers line")
plt.show()
```
▶ What you'll see: raw risk keeps falling with complexity, but the full score rises after cost is added.

### ✍️ Toy 5 · Absolute and relative gaps size the win

A lower score wins, but the gap tells whether the decision is a tiny numerical edge or a meaningful
separation.

```python
import numpy as np                              # scalar gap arithmetic.

t5_rng = np.random.default_rng(0)               # seeded local generator.
t5_baseline = 0.28                              # -> selected score
t5_flexible = 0.34                              # -> alternative score
t5_gap = t5_flexible - t5_baseline              # -> 0.06
t5_relative = t5_gap / t5_flexible              # -> 0.17647058823529416
print("baseline score:", t5_baseline)          # -> 0.28
print("flexible score:", t5_flexible)          # -> 0.34
print("absolute gap:", round(t5_gap, 3))       # -> 0.06
print("relative gap:", round(t5_relative, 3))  # -> 0.176
assert round(t5_gap, 3) == 0.060
assert round(t5_relative, 3) == 0.176

plt.figure(figsize=(4.2, 2.8))
plt.bar(["baseline", "flexible"], [t5_baseline, t5_flexible], color=["seagreen", "indianred"])
plt.ylabel("decision score")
plt.title("Toy 5 · read the score gap")
plt.show()
```
▶ What you'll see: the baseline is lower by `0.060`, about `17.6%` of the flexible score.

### ✍️ Toy 6 · Stabilization lowers the future-facing score

A stabilizing constraint can become a third candidate whose score is lower than both the baseline
and the flexible alternative.

```python
import numpy as np                              # argmin over candidates.

t6_rng = np.random.default_rng(0)               # seeded local generator.
t6_baseline = 0.28                              # -> baseline score
t6_flexible = 0.34                              # -> flexible score
t6_stabilized = 0.80 * t6_baseline              # -> 0.22400000000000003
t6_scores = np.array([t6_baseline, t6_flexible, t6_stabilized])  # -> [0.28  0.34  0.224]
t6_names = np.array(["baseline", "flexible", "stabilized"])  # -> candidates
t6_winner = int(t6_scores.argmin())             # -> 2
print("scores:", np.round(t6_scores, 3).tolist())  # -> [0.28, 0.34, 0.224]
print("winner:", t6_names[t6_winner], round(float(t6_scores[t6_winner]), 3))  # -> stabilized 0.224
assert t6_names[t6_winner] == "stabilized"

plt.figure(figsize=(4.8, 2.8))
plt.bar(t6_names, t6_scores, color=["gray", "indianred", "seagreen"])
plt.ylabel("decision score")
plt.title("Toy 6 · stable candidate wins")
plt.show()
```
▶ What you'll see: the stabilized score `0.224` is the smallest bar.
